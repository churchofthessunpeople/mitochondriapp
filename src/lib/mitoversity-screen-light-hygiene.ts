    ...
    sections: [
      {
        ...
        body: "Intrinsically photosensitive retinal ganglion cells (ipRGCs) containing melanopsin drive circadian phase, melatonin suppression, and alertness. Their action spectrum peaks in blue-cyan (~480 nm). LED backlights and phone OLEDs emit strong short-wavelength content even when content looks “white.” Evening exposure delays melatonin onset and shifts clock phase later in controlled human studies—dose depends on intensity, duration, and spectrum.",
      },
      {
        heading: "Night mode vs aggressive filtering",
        body: "OS “Night Shift” / “Night Light” applies a warm color matrix but often caps how red the image can get—partly for color accuracy and app compatibility. IRIS and f.lux can push further into amber/red, reduce brightness independently, pause on video if configured, and sync to solar time. Trade-off: color-critical work (photo, design) suffers; schedule aggressive modes after work hours or use per-app exceptions.",
      },
      {
        heading: "Multi-device checklist",
        body: "Phone, tablet, laptop, desktop monitor, and TV each need their own setting—forgetting one bright panel resets the signal. Enable automatic sunset schedules tied to location. On iOS: Settings → Display & Brightness → Night Shift (Maximum warmth). Windows: Settings → System → Display → Night light. Android: Display → Eye comfort / Night Light. Mac: System Settings → Displays → Night Shift. Desktop: install IRIS or f.lux if built-in modes feel too blue.",
      },
      {
        heading: "Not a substitute for room light or glasses",
        body: "Warm screens do not fix overhead cool LEDs in the room or street light through curtains. Orange/amber blue-blocking glasses add retinal protection when you must view true-color content. Screen hygiene is one layer in sunset viewing → screen warm → room dim → dark bedroom.",
      },
      {
      ...
      {
        heading: "How this fits the app",
        body: "Log screen light hygiene when evening device use ran under maximum warm-screen settings. Stack with catalog “Evening blue-light hygiene” and “True dark sleep environment.”",
      },
    ],
  },
  advanced: {
    sections: [
      {
        ...
        body: "White LED LCDs use blue pump LEDs with phosphor down-conversion; OLED subpixels emit narrow red, green, and blue with strong 440–490 nm content at “6500 K” white point. Software night modes apply 3×3 color transforms or gamma shifts—reducing melanopic equivalent daylight illuminance (melanopic EDI) but not always below evening targets (~10 melanopic lux is sometimes cited for circadian hygiene; bright phone at “warm” setting can still exceed this). Hardware filters and aggressive LUTs (IRIS “Groot” / extreme red modes) lower melanopic irradiance further at cost of CRI and task performance.",
      },
      {
        heading: "IRIS architecture (representative third-party stack)",
        body: "IRIS intercepts GPU output on Windows/macOS/Linux, applies scheduled color temperature curves, optional PWM flicker reduction heuristics, break reminders, and per-monitor profiles. Solar sync uses civil twilight tables. Compared to f.lux (similar pipeline, long research history in circadian community), IRIS markets deeper red presets for late night. Neither replaces ophthalmic care; both address display metrology, not ambient room SPD.",
      },
      {
        heading: "Circadian phase and melatonin kinetics",
        body: "Evening blue-rich light shifts human phase response curve toward delay; melatonin onset (dim light melatonin onset, DLMO) moves later with sustained exposure. Warm-screen interventions reduce but may not eliminate suppression if luminance stays high. Polysomnography studies link pre-sleep screen use to reduced REM/slow-wave proportions in some cohorts—confounded by content and anxiety. Interpret screen hygiene as reducing melanopic dose, not guaranteeing sleep architecture.",
      },
      {
        heading: "PWM flicker and nnEMF coupling",
        body: "Some panels dim via pulse-width modulation at kHz frequencies—invisible flicker that still stresses sensitive users and couples with Kruse-style nnEMF narratives (time-varying fields plus temporal light modulation). IRIS/f.lux brightness lowering can reduce PWM duty cycle on some hardware. Separate from spectrum: airplane mode and distance address RF; spectrum tools address photon channel.",
      },
      {
      ...
      {
        heading: "How this fits the app",
        body: "Protocol screen-light-hygiene: evening Light-pillar checkbox for display configuration maintained during device use. Points track behavior, not melatonin assays. Educational only.",
      },
    ],
  },
};
